...
One panelist is a spare: if the stuck session is itself running a predefined model, that model
...
first-class interrupt is the real product insight, more than the panel itself.
...
| Requirements / specification review | Humans review a spec for defects before build | The panel does it *mid-build*, on demand, with the code in hand |
...
easy job next to the panelists', it is cheap, and it is fast. But the obvious way to build it
destroys exactly the signal being paid for.

**If three panelists say the same obvious thing and one says something strange and specific, the
strange one is the reason the panel exists.** A consolidator that dedupes and ranks by consensus
buries it as an outlier. Frequency is a *bad* proxy for value here — and doubly so given failure
mode 1, because panelists who read the same anchoring transcript produce **correlated** errors.
...
- **Cluster, never rank by count.** Report how many panelists raised a point as a *label*, not as
  a sort key.
- **Never drop a singleton.** A claim only one panelist made gets shown at full length. If
...
## Failure mode 3 — licensing the panel to doubt the human

The panel's brief should say that the task, spec or handoff it was given **may itself contain a
...
Neither is an agent error. Both are premise errors that a panelist with explicit license would
...
panel faster than any single good catch earns it. The owner's framing was right: *consider* that
...
- The **cold-read panelist matters most here** — the one who never sees the agent's reasoning is
...
2. **Cost and latency.** 4 panelists + a consolidator is 5+ models. Acceptable precisely because
   it is manually triggered and rare — which means it must never fire automatically.
3. **What a panelist returns.** Structured beats prose: *the missed assumption*, *what I would
   check first*, *confidence*, and the separate *premise challenge* slot. Comparable across
   panelists, and it gives the consolidator clean fields to cluster on instead of free text.
4. **Does the stuck session get corrected in place, or abandoned?** Cheapest useful v1: the panel
...
5. **Does the owner ever see the raw panel?** The consolidated view is the default, but a
...
- `mcp__Claude_Code_Remote__create_session` takes a `model` override, a `prompt`, and `tags` —
  so a skill can fan out panelists on four models and tag them for collection. That is the whole
