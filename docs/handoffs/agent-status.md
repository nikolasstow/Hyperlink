...
| **Theme editor** | `feat/theme-editor` | [report](./reports/2026-09-20-theme-editor.md) | **code complete / on-device unverified** | `198aff04` | tsc 0; eslint 0 errors; 1161 tests pass (105 of them never ran before) | `pnpm verify` is red on the base too: 3 toolkit type errors at `examples/ui/file-router/api.ts:78` and `test/document-provide.test-d.ts:24,30`, left for their owner. Created themes are device-local; syncing them through the server config document is the open call. | 2026-09-20 |
