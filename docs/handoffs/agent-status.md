...
| **Code surface Phase 0** | `feat/code-surface-phase0` | [report](./reports/2026-09-21-code-surface-phase0.md) · [handoff](./agent-console-code-surface-phase0.md) | **code complete / on-device unverified** | see PR | both app tsc exit 0; eslint 0 errors; 1192 tests pass; `expo export` lists the asset | Phase 0 is JavaScript only, so no rebuild: monaco, `@shikijs/monaco` and esbuild are devDependencies and `expo-asset` already ships with `expo`. Verified in Chromium against the built asset, not on a device. Custom code fonts stay unrendered until `expo-font` lands, so the surface keeps matching the chat blocks. | 2026-09-21 |
