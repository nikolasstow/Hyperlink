...
| **In-app Builds page** | `app/double-agent/builds` | [report](./reports/2026-09-09-builds-page.md) · [handoff](./builds-page-handoff.md) | **code complete / on-device unverified** | `60db378` | tsc exit 0; 78/78 vitest (44 new) | Two owner calls in the report: **Install** opens the raw `.ipa` per the handoff, which iOS likely cannot install (build-page URL recommended); and list rows show no time because `/builds` carries no `createdAt` (one-line server fix noted). No new deps; `package.json`/lockfile untouched. | 2026-09-09 |
