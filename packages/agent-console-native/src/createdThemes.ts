 ...
 *
 * Storage is on-device, through AsyncStorage, and deliberately not the
 * best-effort pattern `sessionCache.ts` uses. A cache that loses an entry
 * refetches it; a theme someone spent an evening on is gone. Three rules follow
 * from that:
 *
 * 1. A row this build cannot parse is carried through a write untouched.
 *    Dropping it from the list would erase it the next time any other theme was
 *    saved.
 * 2. Writes run one at a time. Every write is a read, a change and a write
 *    back, so two saves in flight would end with the later read overwriting the
 *    earlier save.
 * 3. A failed write raises instead of resolving quietly, because the caller is
 *    a Save button and the person pressing it needs to know.
 *
 ...
 * not. Keeping the second half is what stops a write erasing a theme written by
 ...
 * and writes it back, so overlapping operations would lose whichever landed
 * first. A rejected operation does not stall the ones behind it.
 */
